...
> ⚠️ 若你**先前已經部署過舊版腳本、且 Google Sheet 裡已經有「小隊看板」分頁**：舊分頁的表頭是舊的（只有單一「答錯次數」欄），腳本不會自動改表頭。更新腳本後，請把舊的「小隊看板」分頁**刪除或改名**，讓它下次收到事件時用新表頭重新建立（「事件記錄」分頁不受影響、原始資料都還在）。全新的 Sheet 則不用處理。
...
- [ ] 故意答錯一題 → 「事件記錄」出現 ❌ 與答錯內容、「小隊看板」該關卡錯誤欄 +1、答錯總計 +1
