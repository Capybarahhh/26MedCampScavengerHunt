...
時間、事件（登入／進入關卡／解謎作答／取得碎片／外送遊戲結束／拼圖完成／選擇結局…）、關卡、內容（含答錯的答案原文）、房間碼、sessionId。尚未登入或登入失敗的事件會歸到「未分類」分頁。

**小隊看板** 工作表（一隊一列，活動當天盯這張就好）：
最後活動時間、最新動態、目前關卡、已取得碎片、解謎嘗試、答錯總計、**各關卡答錯次數**（圖書館／舊城區／傳送港／地下市集／垃圾山／市中心／終章各一欄）、外送遊戲、拼圖、結局。

> ⚠️ 若你**先前已經部署過舊版腳本、且 Google Sheet 裡已經有「小隊看板」分頁**：舊分頁的表頭是舊的（只有單一「答錯次數」欄），腳本不會自動改表頭。更新腳本後，請把舊的「小隊看板」分頁**刪除或改名**，讓它下次收到事件時用新表頭重新建立（各小隊自己的分頁不受影響、原始資料都還在）。全新的 Sheet 則不用處理。
