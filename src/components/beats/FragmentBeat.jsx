...
export function FragmentBeat({ beat, onCollect }) {
  ...
  return (
    <div style={css('flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;')}>
      <div style={css('color:#00ffee;font-size:15px;letter-spacing:6px;text-shadow:0 0 10px rgba(0,229,204,0.7);animation:fadeUp 0.5s ease both;')}>FRAGMENT OBTAINED</div>
      ...
      <div style={css('text-align:center;animation:fadeUp 0.5s ease 1s both;')}>
        <div style={css('color:#f0e070;font-size:18px;font-weight:700;letter-spacing:2px;')}>記憶碎片 · {beat.letter}</div>
        <div style={css('color:#c09ad8;font-size:13px;margin-top:6px;letter-spacing:2px;')}>來源：{beat.source}　情緒：{beat.emotion}</div>
      </div>
      ...
      <button
        ...
        onClick={onCollect}
        style={{
          ...css("height:56px;padding:0 32px;background:#2a0e4a;border:2px solid #9a50cc;color:#e0b0ff;border-radius:8px;font-size:15px;letter-spacing:4px;cursor:pointer;"),
