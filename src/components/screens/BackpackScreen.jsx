  ...
  return (
    <div style={css('position:absolute;inset:0;z-index:10;padding:26px 24px;display:flex;flex-direction:column;')}>
      <div style={css('display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;')}>
        <div style={css('color:#c09ad8;font-size:13px;letter-spacing:3px;')}>MNEMO // 記憶背包</div>
        <button className="press96" onClick={onClose} style={css("background:#2a0e4a;border:2px solid #9a50cc;color:#e0b0ff;border-radius:8px;height:40px;padding:0 16px;font-size:12px;letter-spacing:2px;cursor:pointer;")}>◂ 返回</button>
      </div>
      <div style={css('color:#8a6ab0;font-size:12px;letter-spacing:2px;margin:14px 0 24px;')}>已收集 {collectedFragments.length} / 6 枚記憶碎片</div>
        ...
        {FRAGMENT_ORDER.map((letter, i) => {
          ...
          return (
            <div key={letter} style={{
              ...
              background: collected ? '#1a1206' : '#150a24',
              border: `2px ${collected ? 'solid' : 'dashed'} ${collected ? '#f0c030' : '#3a2060'}`,
              boxShadow: collected ? '0 0 20px rgba(240,192,48,0.25), inset 0 0 16px rgba(240,192,48,0.12)' : 'none',
            }}>
              ...
              {collected
                ...
                : <span style={css('color:#3a2060;font-size:36px;position:relative;z-index:1;')}>?</span>}
            </div>
          );
        })}
      </div>
